...
@implementation WYChineseToPinyinTools : NSObject

+ (NSString *)chineseToPinyinWithChinese:(NSString *)chinese {
	if(!chinese || ![chinese length]) return nil;
	
	NSStringEncoding enc = CFStringConvertEncodingToNSStringEncoding( kCFStringEncodingGB_18030_2000);
	NSData *gb2312_data = [chinese dataUsingEncoding:enc];
...
@implementation WYWord
@end

@interface WYPinyinSort ()
    ...
    [_chineses enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([predicate evaluateWithObject:initialString]) {
            ...
            if (![word.formatChinese isEqualToString:@""]) {
                word.pinyin = [WYChineseToPinyinTools chineseToPinyinWithChinese:word.formatChinese];
...
- (void)sort {
    ...
    [self.words enumerateObjectsUsingBlock:^(WYWord *obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
+ (instancetype)defaultPinyinSort {
    static WYPinyinSort *_s = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _s = [[WYPinyinSort alloc] init];
    });
    return _s;
}
